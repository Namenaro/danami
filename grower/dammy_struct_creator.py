...
def get_dammy_struct_and_realisation_from_events_list(cogmap_events_ids_list, cogmap):
    ...
    for local_event_id in graph.recognition_order[1:]:
        ...
        parent_point = cogmap.get_point_of_event(parent_local_id)
        child_point = cogmap.get_point_of_event(local_event_id)
        ...
        struct_memory.add_new_event(event_memory, u_from_parent, parent_global_id, is_linked_to_parent)
...
def fill_dammy_event_memory_from_cogmap_event(cogmap, local_event_id):
    ...
    event_stat = None
    event_inner_vals = InnerEventVals(zmeyka=zmeyka)
